...
@onready var animacaodivisao = $"BossFinal"/AnimatedSprite2D


func _ready() -> void:
	correct = 0
	mana = 0
	load_stats()                      # ← carregar as vidas do txt
...
# ============================================================
func load_stats():
	var file = FileAccess.open("res://statsbossfinal.txt", FileAccess.READ)

	if file == null:
		push_error("❌ ERRO: statsbossfinal.txt não encontrado!")
		vida = 3
		vidaboss = 10
		return
...
func dar_dano() -> void:
	if mana == 3:
		...
		animacaodivisao.play("tomano")
		vidaboss -= 1
		print("Vida do Boss: ", vidaboss)
		mana = 0
